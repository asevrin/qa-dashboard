...
function snapshotDetails(caseItem) {
  ...
  return `<div class="problem-case-content"><div class="snapshot-properties"><span>${escapeHtml(caseItem.priority)} priority</span><span>${escapeHtml(caseItem.executionScope)} scope</span></div><div class="snapshot-grid"><section><h3>Preconditions</h3><p>${escapeHtml(caseItem.preconditions || "None")}</p></section><section><h3>Expected result</h3><p>${escapeHtml(caseItem.expectedResult || "Not specified")}</p></section></div><section class="snapshot-section"><h3>Steps</h3>${steps}</section>${caseItem.notes ? `<section class="snapshot-section"><h3>Notes</h3><p>${escapeHtml(caseItem.notes)}</p></section>` : ""}<section class="snapshot-section execution-note"><h3>Execution result</h3><p>${caseItem.resultComment ? escapeHtml(caseItem.resultComment) : "No comment provided."}</p></section>${artifactList(caseItem)}</div>`;
}
